...
const PerformanceGraph = () => {
  ...
  useEffect(() => {
    const fetchPerformanceData = async () => {
      try {
        ...
        const formattedData = response.stats.map(stat => ({
  ...
  subtitle={`${stats.totalCorrect} of ${stats.totalPredictions} total predictions`}
  color="green"
  tooltipText="Total correct predictions divided by total number of predictions across all time. This is our most accurate measurement of overall performance."
/>
        <StatCard
          title="Average Daily"
          value={`${stats.avg.toFixed(1)}%`}
          subtitle={`Average of daily percentages`}
          color="blue"
          tooltipText="Average of daily accuracy rates. This might differ from overall accuracy as days with few predictions carry equal weight to days with many predictions."
        />
        <StatCard
          title="Best Day"
